...
$("#grd_registro_pagos").hideColumn(8);
function action(newVal, oldVal) {

    $("#grd_registro_pagos").hide();
    $("#grd_registro_pagos").disableValidation();

    if (newVal == "CONTINUAR") {
        $("#grd_registro_pagos").show();
        $("#grd_registro_pagos").enableValidation();
...
action($("#frm_ac_accion").getValue(), '');
$('#frm_ac_accion').setOnchange(action);