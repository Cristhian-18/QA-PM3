...
//if contains 60, replace with 59
if (strpos(@@frm_busqueda_horaSiniestro, "60") !== false) {
    $hora_reporte = str_replace("60", "59", @@frm_busqueda_horaSiniestro);
...
echo 'Incializar Datos Solicitud';
...
$sql_cata_poli = "SELECT DESCRIPCION FROM ADMIN_CATALOGOS WHERE CODIGO = 'Consultar_poliza_Placa_IdPv'";
    ...
    if($key == 'coberturas'){
        ...
        foreach($data as $datacob){
            ...
            if(@@frm_cobertura_aplicada == $datacob->codCobertura){
                $cob_apli = 'SI';
                echo "Cobertura Aplicada 1: ".@@frm_cobertura_aplicada;
                print_r($datacob);
                if(strpos($datacob->cobertura, 'RESPONSABILIDAD CIVIL') !== false){
                    ...
                    @@nombre_cobertura = $datacob->cobertura;
            ...
            echo "Cobertura Aplicada 2: ".@@frm_cobertura_aplicada;
        }
        $aux_sini = 1;
        if($key == 'siniestros'){
