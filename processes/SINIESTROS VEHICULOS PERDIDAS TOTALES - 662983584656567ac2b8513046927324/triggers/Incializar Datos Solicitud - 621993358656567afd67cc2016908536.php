...
$sql_cata_condicionesPoliza = "SELECT DESCRIPCION FROM ADMIN_CATALOGOS WHERE CODIGO = 'Consultar_Condiciones_Poliza'";
...
if (empty($idPv) || $idPv == 0) {
    echo 'IDPV no disponible, se omite consulta de condiciones de póliza.';
}else{
    try {
        $ch = curl_init();
...
//PAIS
$sql_cata_infoPais = "SELECT DESCRIPCION FROM ADMIN_CATALOGOS WHERE CODIGO = 'Catalogo_Pais'";
...
$sql_cata_infoAgente = "SELECT DESCRIPCION FROM ADMIN_CATALOGOS WHERE CODIGO = 'Consultar_agente'";
