...
//Solicitante
@@tri_user_auditor = $aCase['APP_DATA']['tri_user_auditor'];
...
foreach ($auditor as $groupArray) {
    if ($groupArray['USR_UID'] == $current_user) {
        $usr_uid = $groupArray['USR_UID'];
        echo 'Same user';
...
$sql = "SELECT CODIGO FROM ADMIN_CATALOGOS WHERE
     COD_CATALOGO = 'AJUSTADORES_PROVINCIA' 
