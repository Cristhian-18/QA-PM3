...
$pro_uid = '42420581765087d55c278c7052161956';
$username = @@frm_datosSolicitud_solicitante;
@@tri_bandera_mda = '';
@@tri_bandera_mda_comercial = 'false';
@@tri_bandera_mda_tecnico = 'false';
@@tri_bandera_evaluacion = 'false';

@@frm_datosSolicitud_solicitante_uid = UidUsuario($username);
	...
	foreach ($arr_resul as $keydata => $data) {
		if ($keydata == 'COMERCIAL') {
			if ($data == 'EJECUTIVO' || $data == 'COMITE') {
				@@tri_user_mda_comercial = @@frm_datosSolicitud_solicitante_uid;
				if ($data == 'COMITE')
