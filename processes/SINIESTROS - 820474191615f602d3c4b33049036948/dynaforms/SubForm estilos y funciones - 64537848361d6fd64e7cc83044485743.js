...
function showConfirmDlg() {
	...
	$("div.pmdynaform-edit-text, div.pmdynaform-edit-dropdown, div.pmdynaform-edit-datetime, div.pmdynaform-edit-file, pmdynaform-edit-dropdown, pmdynaform-edit-file").each(function(index, element) {
//      $("div.pmdynaform-edit-datetime").each(function(index) {
		...
		stringHtml = $(this).getControl().attr("style").toString();
  //    	alert(($(this).toString()));
		...
		if((deshabilitadaValidacion && $(this).find(".pmdynaform-field-required").html() == '*') || noCumplioValidacion || noCumplioValidacionProcess){
          if($(this).attr('class').indexOf('file') > -1){
            if($(this).getText(0).length == 0){
                count++;
				c += count + ") " + $(this).getLabel() + "<br>";
              	$(this).getControl().css("border", "1px solid red");
				bandera = false;
             }else{
