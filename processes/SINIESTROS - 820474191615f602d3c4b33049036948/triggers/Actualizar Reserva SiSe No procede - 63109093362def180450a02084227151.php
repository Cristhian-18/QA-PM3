...
try{
    ...
    switch($task){
        ...
        $dyn = "19704822661d89a84dc5eb6067966042";
        break;
        //tarea 4
        case "359772973624db81b5141e6050784057":
        $dyn = "669886330625d96ef6d7cd1073394695";
        break;
        default:
        $dyn = "19704822661d89a84dc5eb6067966042";
        break;
    ...
    $marca_stro = '2';
    if(@@tri_bandera_update_aux == 'true')
    $marca_stro = '4';
...
if(curl_errno($ch_auth)){
    ...
    //tarea 2
    //PMFRedirectToStep(@@APPLICATION, @%INDEX, 'DYNAFORM', $dyn);
}
curl_close($ch_auth);
$rs_m_auth = json_decode($res_auth,true);
 
PMFBitacoraServicios(@@APP_NUMBER, 'trigger', 'Actualizar reserva sise No procede', $dns_auth, 'POST', 'NO' , json_encode($json_auth), json_encode($rs_m_auth), $msg_m_auth);
...
if(@@grd_coberturas['1']['grd_txt_desc_riesgo'] != '' ){
    ...
    PMFBitacoraServicios(@@APP_NUMBER, 'trigger', 'Actualizar reserva sise No procede', $dns, 'POST', 'Bearer '.$token, json_encode($json_stro), json_encode($result), $msg_m);
    @@result_update_baja = $result;

    if(isset($result['isError']) && $result['isError'] === true){
        $msg_result = isset($result['Message']) ? $result['Message'] : '';
        $err_number = isset($result['errorNumber']) ? $result['errorNumber'] : '';

        //Caso de negocio "estimacion menor al monto pagado": se informa pero el caso debe avanzar, no se redirige al dynaform.
        $es_estimacion_menor_monto_pagado = ($err_number == 1 && stripos($msg_result, 'no puede ser menor al monto pagado') !== false);

        if($es_estimacion_menor_monto_pagado){
            @@tri_message_update = $msg_result . "\n" . 'VUELVA A INTENTAR, NO SE OLVIDE DE ACTUALIZAR EL SISE';
            @@bandera_sise = 'true';
            
        }else{
            @@tri_message_update = $msg_result . ' - VUELVA A INTENTAR, NO SE OLVIDE DE ACTUALIZAR EL SISE';
            @@bandera_sise = 'true';
            PMFRedirectToStep(@@APPLICATION, @%INDEX, 'DYNAFORM', $dyn);
        }
    }
    ...
    if ($swApi == 'false'){
        ...
        if(curl_errno($ch)){
            $msg_m = curl_error($ch);
            /*if($usrSeg == 'jvalenzuela' || $usrSeguimiento == 'jvalenzuela'){
                echo '<br>Error 3: '.$msg_m;
                //die();
            }*/
            /*}
            curl_close($ch);
            ...
            /*if (is_object($result)){
                ...
                $noResult = true;
                //echo "<br>no es ni objeto ni array";
                //throw new Exception("Respuesta inválida del servicio: no es JSON válido");
            }

            //print_r($resultArray);
            //die();
            /*if($noResult == false){
                $errorNumber = $isObject ? $result->errorNumber : $resultArray['errorNumber'];
                echo '<br>ErrorNumber: '.$errorNumber;
                $id_stro     = $isObject ? $result->id_stro     : $resultArray['id_stro'];
                echo '<br>Id_stro: '.$id_stro;
                $nro_stro    = $isObject ? $result->nro_stro    : $resultArray['nro_stro'];
                echo '<br>Nro_stro: '.$nro_stro;
                $Message     = $isObject ? $result->Message     : $resultArray['Message'];
                echo '<br>Message: '.$Message;

                if($errorNumber == 0)
                {
                    @@tri_id_stro_up = $id_stro;
                    @@tri_nro_stro_up = $nro_stro;
                    ...
                    @@tri_message_update = $Message.' - VUELVA A INTENTAR, NO SE OLVIDE DE ACTUALIZAR EL SISE';
                    @@bandera_sise = 'true';
                    /*if($usrSeg == 'jvalenzuela' || $usrSeguimiento == 'jvalenzuela'){
                        echo '<br>Dynaform 2: '.$dyn;
                        echo '<br>Error 4: '.$Message;
                        //die();
                    }
                    //PMFRedirectToStep(@@APPLICATION, @%INDEX, 'DYNAFORM', $dyn);
            ...
            /*}catch(Exception $e)
            {
                $resultError = is_array($result) ? $result : (array)$result;
                $resultError['mensaje'] = 'false';
