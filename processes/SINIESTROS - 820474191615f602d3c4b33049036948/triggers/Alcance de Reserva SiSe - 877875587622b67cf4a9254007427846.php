...
try{
    ...
    $rs_ca = executeQuery($sql_ca, $cnx_rp);
...
PMFBitacoraServicios(@@APP_NUMBER, 'trigger', 'Actualizar reserva sise', $dns_auth, 'POST', 'NO', json_encode($json_auth), json_encode($rs_m_auth), json_encode($msg_m_auth));
...
PMFBitacoraServicios(@@APP_NUMBER, 'trigger', 'ARS-S-288', $dns, 'POST', 'NO', json_encode($json_stro), json_encode($result), $msg_m);
...
if(isset($result->isError) && $result->isError === false && isset($result->errorNumber) && $result->errorNumber == 0)
{
    ...
    if(@@TASK == '309930261615f607b901f74034966395'){
        ...
        if(@@TASK == '799986505615f607b50a4f4033464318'){
            PMFRedirectToStep(@@APPLICATION, @%INDEX, 'DYNAFORM', '799986505615f607b50a4f4033464318');
        }else{
            PMFRedirectToStep(@@APPLICATION, @%INDEX, 'DYNAFORM', '76587094661da3be8a7b6b9083070571');
        }
    }
