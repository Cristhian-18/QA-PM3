...
try {
    ...
    *******************************************************/
    ...
    $tmp_poliza_aux = @@frm_fecha_ingreso_poliza_aux;

    if ($tmp_poliza_aux == '') {
        @@frm_fecha_ingreso_poliza_aux = @@frm_fecha_ingreso_poliza;
    }
    ...
    if (@@frm_accion == 'CONTINUAR' || @@frm_accion == 'ESPERAR') {
        if (@@tri_bandera_alcance == 'ALCANCE') {
            ...
            $tmp_tipo_pago = @@frm_tipo_pago;

            if ($tmp_tipo_pago == '') {
                $cod_tipo_pago = 0;
            } else {
                $cod_tipo_pago = $tmp_tipo_pago;
            }
            $nro_crm             = @@APP_NUMBER;
            ...
            $tmp_mes_vencido = @@frm_mes_vencido_label;

            if ($tmp_mes_vencido == 'true') {
                $sn_mes_vencido = 1;
            } else {
                $sn_mes_vencido = 0;
            }
            ...
            $monto_reportado = (float) @#frm_monto_reportado;



            if (@#frm_monto_pagado_al == '') 
            {
                $monto_pagado = 0;
            } else {
                $monto_pagado = (float) @#frm_monto_pagado_al;
            }

            $imp_estimado = $monto_reportado + $monto_pagado;
            ...
            PMFBitacoraServicios(@@APP_NUMBER, 'trigger', 'CUS-S-225', $dns_auth, 'POST', '',  $json_auth, $res_auth, $msg_m_auth);

            $token = '';
            try
            {
                if (is_array($rs_m_auth) && count($rs_m_auth) > 0) {
            ...
            //detalle_cobertura
            if (isset($grd_coberturas[1]['grd_txt_desc_riesgo']) && $grd_coberturas[1]['grd_txt_desc_riesgo'] != '') {
                $i = 1;
            ...
            $grd_beneficiarios = @@grd_beneficiarios;

            if (isset($grd_beneficiarios[1]['grd_codigo_b']) && $grd_beneficiarios[1]['grd_codigo_b'] != '') {
            ...
            try {
                ...
                PMFBitacoraServicios(@@APP_NUMBER, 'trigger', 'CUSAC', $dns, 'POST', "Authorization: Bearer " . $token,
                    json_encode($json_stro), json_encode($result), json_encode($msg_m));
                ...
                if (isset($result->isError) && $result->isError === false && isset($result->errorNumber) && $result->errorNumber == 0) {
                    ...
                    @@tri_message_grabado   = $msg_m_auth;
                    $g                      = new G();
                    $g->SendMessageText(@@tri_message_update_al, "WARNING");
                    @@envio_mail = PMFSendMessage(@@APPLICATION, $de, $para, $cc, $bcc, $asunto, $plantilla_rec, ['texto' => $texto]);
                }
            ...
            $cod_cobertura_madre = @@frm_cobertura_madre;
            $tmp_diag            = @@frm_diagnostico;

            if ($tmp_diag == '') {
                $cod_diagnostico = 0;
            } else {
                $cod_diagnostico = $tmp_diag;
            }
            $cod_motivo_detenido  = 1;
            $cod_usuario_autoriza = "DSAA";
            //$fec_concesion_credito = (@@frm_fecha_concesion == '' ? '1999-01-01' : @@frm_fecha_concesion);

            $tmp_fec_con = @@frm_fecha_concesion;

            if ($tmp_fec_con == '') {
                $fec_concesion_credito = '1999-01-01';
            } else {
                $fec_concesion_credito = $tmp_fec_con;
            }

            //$fec_vencimiento_credito = (@@frm_fecha_vencimiento == '' ? '1999-01-01' : @@frm_fecha_vencimiento);
            $tmp_fec_ven = @@frm_fecha_vencimiento;

            if ($tmp_fec_ven == '') {
                $fec_vencimiento_credito = '1999-01-01';
            } else {
                $fec_vencimiento_credito = $tmp_fec_ven;
            }

            //$cod_plazo_credito = (@@frm_plazo_credito == '' ? 0 : @@frm_plazo_credito);
            $tmp_plazo = @@frm_plazo_credito;

            if ($tmp_plazo == '') {
                $cod_plazo_credito = 0;
            } else {
                $cod_plazo_credito = $tmp_plazo;
            }
            $cod_aseg = @@frm_cod_aseg;
            ...
            $rs_m_auth = json_decode($res_auth, true);
             ...
             PMFBitacoraServicios(@@APP_NUMBER, 'trigger', 'CUS-S-556', $dns_auth, 'POST', '',  $json_auth, $rs_m_auth, $msg_m_auth);
            ...
            try {
                ...
                $result = json_decode($res);
                echo '11';
                PMFBitacoraServicios(@@APP_NUMBER, 'trigger', 'CUS-S-747', $dns, 'POST', "Authorization ", $json_stro, json_encode($result), $msg_m);
                ...
                if (isset($result->isError) && $result->isError === false && isset($result->errorNumber) && $result->errorNumber == 0) {
                    ...
                    @@tri_message_grabado = $result->Message;
                    $g                    = new G();
                    $g->SendMessageText(@@tri_message_grabado, "WARNING");
                    @@envio_mail = PMFSendMessage(@@APPLICATION, $de, $para, $cc, $bcc, $asunto, $plantilla_rec, ['texto' => $texto]);
                    if (@@TASK == '799986505615f607b50a4f4033464318') {
