...
try {
    ...
    $Nombre = @@frm_apellido_paterno . ' ' . @@frm_apellido_materno . ' ' . @@frm_nombres;
    $Tipo = 1;
    $SubTipo = "VEHICULOS";
    ...
    if (is_array($outDoc)) {
        ...
        foreach ($outDoc as $dataoutDoc) {
            ...
            echo "<p>" . $name . "</p>";
            switch ($name) {
                case "288996193625f705922b301060222767":
                    $TipoDocumento = "OTROS_DOCUMENTOS";
                    break;
                case "49334809561f2952b885af3088578354":
                    $TipoDocumento = "OTROS_DOCUMENTOS";
                    break;
                case "89592389462588bff5884c6085554259":
                    $TipoDocumento = "OTROS_DOCUMENTOS";
                    break;
                case "555155095624e041abfbdd9056360623":
                    $TipoDocumento = "OTROS_DOCUMENTOS";
                    break;
                default:
                    $TipoDocumento = "OTROS_DOCUMENTOS";
    ...
    if (is_array($outDoc)) {
        ...
        foreach ($outDoc as $dataoutDoc) {
            ...
            try {
                ...
                $result = json_decode($res);
                echo '15';
                PMFBitacoraServicios(@@APP_NUMBER, 'trigger', 'Guardar documentos MFILES', $url_mfils_param, 'POST', "apikey: " . $token, $json, $result, $msg_m);
            } catch (Exception $e) {
                //echo 'ExcepciÃƒÂ³n capturada: ',  $e->getMessage(), "\n";
                $result['mensaje'] = 'false';
                $result['mensaje_mostrar'] = 'Excepción capturada: Error al consultar la Base de Datos, comuniquese con el administrador.- ' . $e->getMessage();
