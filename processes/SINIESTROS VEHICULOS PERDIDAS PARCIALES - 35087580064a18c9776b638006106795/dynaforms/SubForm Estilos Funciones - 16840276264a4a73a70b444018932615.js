...
function validarExpresionRegular(id, numeroExpresion){
	if(numeroExpresion == 4){ ponerDecimales(id); }
	$("#" + id).focusout(function(){ validarExpresionRegular_2($(this), numeroExpresion); }); 	
...
function limitMaxMin(_float, _max, _min){
  ...
  if(parsed > _max) { return _max; }
  if(parsed < _min) { return _min; }
  return parsed;
}
...
function ponerDecimales(id){
	...
	if(valor == ''){ identificador.setValue(""); return; }  
...
function validarIdentificacion(identificacion, tipoIdentificacion, aux) {
    ...
    if (tipoIdentificacion == 'C') {
        if (numero.length != 10) {
            if (aux) { alert("La identificacion no tiene diez digitos"); }
            identificacion.value = '';
            bandera = false;
            return false;
        }
    } else if (tipoIdentificacion == 'R') {
      	if (isNaN(numero)) { alert("La identificacion es incorrecta"); }
        if (numero.length != 13) {
            if (aux) { alert("La identificacion no tiene trece digitos"); }
            identificacion.value = '';
            bandera = false;
            return false;
        }
        if (tercerDigito == 9) {
            ...
            totdigitos = 10; total = 0;
            digito = (digitos[9] * 1);
            p1=digitos[0]*4; p2=digitos[1]*3; p3=digitos[2]*2;
            p4=digitos[3]*7; p5=digitos[4]*6; p6=digitos[5]*5;
            p7=digitos[6]*4; p8=digitos[7]*3; p9=digitos[8]*2;
            total = p1+p2+p3+p4+p5+p6+p7+p8+p9;
            residuo = total % 11;
            final = residuo == 0 ? 0 : 11 - residuo;
            if (final == digito) { bandera = false; return true; }
            else {
                if (aux) { alert("RUC EMPRESA no valido"); }
                identificacion.value = ''; bandera = false;
            }
        }
    }
    if (bandera) {
        ...
        for (i = 0; i < (totdigitos - 1); i++) {
            mult = 0;
            if ((i % 2) != 0) { total = total + (digitos[i] * 1); }
            else {
                mult = digitos[i] * 2;
        ...
        if ((final == 10 && digito == 0) || (final == digito)) {
            ...
            if (tipoIdentificacion == 'P') { return true; }
            else {
                if (aux) { alert("Identificacion no valida"); }
                identificacion.value = '';
                return false;
            }
        }
...
function clearGrid(grd_name) {
    ...
    for (var i=1; i < rows; i++) { $("#"+grd_name).deleteRow(); }
    var aValues = $("#"+grd_name).getValue();
    for (var i=1; i <= aValues[0].length; i++) { $("#"+grd_name).setValue("", 1, i); }
}