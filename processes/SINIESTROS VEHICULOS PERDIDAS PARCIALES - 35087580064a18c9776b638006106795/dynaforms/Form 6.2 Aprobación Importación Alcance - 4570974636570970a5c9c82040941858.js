...
$("#grd_valores_siniestros_alcance").hideColumn(5);
$("#grd_valores_siniestros_alcance").hideColumn(7);
$("#grd_valores_siniestros_alcance").hideColumn(8);
...
for (let i = 0; i <= length; i++) {
    $("#grd_valores_siniestros_alcance").getControl(i+1, 3).attr('disabled', true);
    $("#grd_valores_siniestros_alcance").getControl(i+1, 4).attr('disabled', true);
    $("#grd_valores_siniestros_alcance").getControl(i+1, 5).attr('disabled', true);
    $("#grd_valores_siniestros_alcance").getControl(i+1, 6).attr('disabled', true)
    $("#grd_valores_siniestros_alcance").getControl(i+1, 7).attr('disabled', true)
    $("#grd_valores_siniestros_alcance").getControl(i+1, 8).attr('disabled', true)
    if(($("#grd_valores_siniestros_alcance").getValue(i+1, 11))!="Pendiente"){
        $(rows[i]).hide();
    } else {
        $("#grd_valores_siniestros_alcance").getControl(i+1, 11).attr('required', true)
        $(negados[i]).hide();
        $(pendientes[i]).hide();
    }
}
