...
$("#grd_valores_siniestros").hideColumn(11);
...
$("#10895930265393d60a0d399090770268").setOnSubmit(function(){
  var aRc = $("#grd_valores_siniestros").getNumberRows();
  if($("#frm_accion").getValue() == "CONTINUAR"){
  for (var i=0; i <= aRc; i++) {
      if ($("#grd_valores_siniestros").getValue(i, 1) != "" 
      && $("#grd_valores_siniestros").getValue(i, 2) != ""
      && $("#grd_valores_siniestros").getValue(i, 3) != "") {
          if($("#grd_valores_siniestros").getValue(i, 7) == "" || 
            $("#grd_valores_siniestros").getValue(i, 7) == null){
              alert("Verifique la cotización de todos los repuestos");
              return false;
          }
      }
  }}
...
function action(newVal, oldVal) {
  $('#tri_user_mundopartes').hide();
    $('#grd_valores_siniestros').enableValidation();

  if (newVal == "MANTENER") {
    $('#tri_user_mundopartes').show();
        $('#grd_valores_siniestros').disableValidation();
