...
let array_objetos = ['frm_asegurado_tipo', 'frm_conductor_identificacion', 'frm_conductor_nombres', 'frm_conductor_telefono', 'frm_accidente_pais', 'frm_accidente_provincia', 'frm_accidente_ciudad', 'frm_siniestro_direccion', 'frm_siniestro_detalle', 'frm_danios_detalle']


array_objetos.forEach(element => {
    let value = $('#' + element).getValue()
    //value = value.trim()
    if (value != '') {
        //$("#frm_datosSolicitud_cliente").getControl().attr('disabled', true);
        $('#' + element).getControl().attr('disabled', true);
    } else {
        $('#' + element).getControl().attr('disabled', false);
    }
});

$('#frm_siniestro_OtrosVehiculos').getControl().attr('disabled', true);
$('#frm_siniestro_Propiedad').getControl().attr('disabled', true);
$('#frm_siniestro_Personas').getControl().attr('disabled', true);
...
$('#file_documentosAnalisis').hide();
if ($('#PROCESS').getValue() == '76661804465655b4bdffbc2081200894') {
    $('#fle_denuncia').hide();
}
...
$('.menu').on('click', function () {
    ...
    switch (this.id) {
        ...
        case 'documentos':
            $('#subt_docs').show()
            $('#2450178376694c8b8e1d294076599593').show()
            break
        case 'historial':
            $('#sbt_historial').show()
...
function ocultar_todo() {
    ...
    $('#subt_docs').hide()
    $('#2450178376694c8b8e1d294076599593').hide()
    $('#sbt_historial').hide()
...
function ocultarBotonAgregarVehiculosAfectados() {
    // Oculta botón/label "Agregar" del grid de Vehículos afectados
    $('#grd_vehiculos_afectados')
        .find('.pmdynaform-grid-text-plus')
        .closest('button, a, .pmdynaform-grid-new, .pmdynaform-grid-add')
        .hide();

    // Fallback por si solo existe el span
    $('#grd_vehiculos_afectados .pmdynaform-grid-text-plus').hide();
}
...
ocultar_todo()
mostrar_solicitud()
ocultarBotonAgregarVehiculosAfectados()
