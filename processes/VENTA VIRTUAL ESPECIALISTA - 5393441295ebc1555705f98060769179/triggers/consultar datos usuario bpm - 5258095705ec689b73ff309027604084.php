...
@@frm_vendedor_cargo = $rs[1]['USR_POSITION'];
...
// consultar si es usuario broker
$sqlgrp = "SELECT GRP_UID FROM GROUP_USER WHERE USR_UID = '$usuario' AND GRP_UID = '648316060609d915351ece8074444881'";
$rsgrp = executeQuery($sqlgrp);
@@TMP_SQLGRP = $sqlgrp;
@@tri_es_broker = (count($rsgrp) > 0 ? 'SI' : 'NO');
...
//CAMBIOS EN LA SOLICITUD DE SEGURO
@@tri_solicitud_siglas = 'SS';
@@tri_solicitud_siglas_fv = 'FV';
@@tri_solicitud_persona = 'PN';
@@tri_solicitud_anio = date('Y');
@@tri_solicitud_num = @@APP_NUMBER;

@@tri_solicitud_label = @@tri_solicitud_siglas.'-'.@@tri_solicitud_persona.'-'.@@tri_solicitud_anio.'-'.@@tri_solicitud_num;
@@tri_solicitud_label_fv = @@tri_solicitud_siglas.'-'.@@tri_solicitud_persona.'-'.@@tri_solicitud_anio.'-'.@@tri_solicitud_num;
