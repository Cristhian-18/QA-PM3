...
foreach($rs as $data){
	...
	switch($cod){
		...
		break;
		case "SUSCRIPCION":
			if(isset(@@frm_accion_suscripcion) && @@frm_accion_suscripcion != 'APROBAR'){
				$arr_politicas[$aux]['frm_polemi_descripcion'] = $data['DESCRIPCION'];
				$arr_politicas[$aux]['frm_polemi_resultado'] = $data['VALOR'];
				$arr_politicas[$aux]['frm_polemi_obs'] = $data['CAMPO2'];
				$bandera = false;
		...
		break;
		case "EMISION":
			if(isset(@@frm_accion_emisiona) && @@frm_accion_emisiona == 'ERROR'){
				$arr_politicas[$aux]['frm_polemi_descripcion'] = $data['DESCRIPCION'];
				if (@@entradaT10 >= 1){
		...
		break;
		case "TARIFA":
			if(isset(@@frm_accion_emision) && @@frm_accion_emision == 'CONTINUAR'){		
				if (isset(@@tri_decision_magnum_result) && @@tri_decision_magnum_result == 'REFER'){
					$arr_politicas[$aux]['frm_polemi_descripcion'] = $data['DESCRIPCION'];
					$arr_politicas[$aux]['frm_polemi_resultado'] = $data['VALOR'];
					$arr_politicas[$aux]['frm_polemi_obs'] = $data['CAMPO2'];
				}else{
