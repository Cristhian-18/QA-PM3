...
$query = "SELECT
  ...
  APP_DOC_FILENAME AS COMENTARIO,
  USR_UID,
  DOC_UID,
  APP_DOC_UID,
  DOC_VERSION
...
AND APP_DOC_TYPE='OUTPUT' ORDER BY APP_DOC_CREATE_DATE DESC";
...
$query_i = "SELECT APP_DOC_UID, APP_DOC_CREATE_DATE AS FECHA, USR_UID, APP_DOC_COMMENT AS COMENTARIO, DOC_VERSION, APP_DOC_FILENAME AS FILENAME, APP_DOC_FIELDNAME
                FROM APP_DOCUMENT
                WHERE APP_UID='$case_id'
                AND APP_DOC_TYPE IN ('INPUT','ATTACHED') AND APP_DOC_STATUS = 'ACTIVE' AND APP_DOC_FIELDNAME NOT IN ('file_cotizacion_csv','file_cotizacion')
