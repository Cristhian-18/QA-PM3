...
$("#frm_cliente_edad").setValue(calcular_edad($("#frm_fecha_nacimiento").getValue()));
...
function checkCountry(newVal, oldVal) {
  if(newVal != ''){
	  $("#frm_nacionalidad").setValue(newVal);
  }
  if (newVal == 56) {
    $("#frm_ciudad_nacimiento").show();
    $("#frm_ciudad_nacimiento_otro").hide();
  } else {
    $("#frm_ciudad_nacimiento").hide();
    $("#frm_ciudad_nacimiento_otro").show();
  }
}
//execute when the Dynaform loads:
if ($("#frm_pais_nacimiento").getValue() != '') {
  checkCountry($("#frm_pais_nacimiento").getValue(), '');
} else {
  $("#frm_ciudad_nacimiento").hide();
  $("#frm_ciudad_nacimiento_otro").hide();
}
//execute when field's value changes:
$('#frm_pais_nacimiento').setOnchange(checkCountry);

//CAMBIAR DIRECTO LA NACIONALIDAD
$('#frm_pais_nacimiento').setOnchange(checkCountry);
