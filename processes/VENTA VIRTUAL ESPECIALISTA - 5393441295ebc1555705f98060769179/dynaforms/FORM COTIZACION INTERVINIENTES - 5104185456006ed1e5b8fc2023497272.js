...
function estadoCivil() {
  ...
  if (estado == 5 || estado == 2) {
    ...
    $("#frm_conyuge_apellido_materno").disableValidation();
    $("#frm_conyuge_primer_nombre").disableValidation();
    $("#frm_conyuge_segundo_nombre").disableValidation();
...
$("#5104185456006ed1e5b8fc2023497272").setOnSubmit(function () {
  $("#5104185456006ed1e5b8fc2023497272").saveForm();
  console.log('1');
  var pagador = $("#frm_pago_terceros").getControl().val();
...
function consultar_equifax_cliente() {
  ...
  if (cedula != '' && frecuencia != '') {
    $.ajax({
      ...
      dataType: 'json',
      beforeSend: function () {
        // $("#5104185456006ed1e5b8fc2023497272").showFormModal();
      },
      success: function (respuesta) {
        ...
        if (respuesta.mensaje != 'false') {
          ...
          if (calificacion == 'E') {
            var mensaje = tarjeta + " Tiene tarjeta de credito: Este cliente no es pagador apto; requiere obligatoriamente pagador tercero";
            var color = "red";
            var estado = "NOPASA";
            //  $("#frm_pago_terceros").setValue('S');
            //  $("#frm_pago_terceros").getControl().attr('disabled', true);
